...
import { Shield, Camera, Flame, Droplets, Home, Lock, Briefcase } from 'lucide-react';
...
const BoligalarmCategoryIcons: React.FC<BoligalarmCategoryIconsProps> = ({
  ...
  const categories = [
    ...
    {
      id: 'smartlås',
      label: t('alarm.smartlock'),
      icon: Lock,
      description: t('alarm.smartlockDescription')
    },
    {
      id: 'bedrift',
      label: t('alarm.business'),
      icon: Briefcase,
      description: t('alarm.businessDescription')
    }
  ];

  const handleCategoryClick = (categoryId: string) => {
    const newCategory = categoryId === 'all' ? null : categoryId;
    console.log('Boligalarm category selected:', newCategory);
  ...
  return (
    <div className="mb-6 sm:mb-8 max-w-full overflow-hidden">
      <h3 className="text-base sm:text-lg font-semibold text-foreground mb-3 sm:mb-4 px-1">{t('alarm.selectCategory')}</h3>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-8 gap-2 sm:gap-3 lg:gap-4 max-w-full">
